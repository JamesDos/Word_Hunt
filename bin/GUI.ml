...
let normal_play_time = 60
let survival_play_time = 20
let seconds_to_warn = 5
let exit_on_escape = true

(* debug *)

let debug_accept_every_word = false
...
let main () =
  ...
  *)
  ...
  let page1 =
    L.tower ~hmargin:500
      [
        ...
        L.resident ~w:1000 ~h:100 quit_intro_button;
      ]
  in
  ...
  (* [is_word_incomplete word] returns true if the word in the entry field
     can be added to the successful word list; false otherwise *)
  let is_word_incomplete word =
    (not debug_accept_every_word)
    && ((not (GameBoard.is_valid_word (List.rev !entered_locs) !board))
       || List.mem word !entered_words)
  in

  (* [color_word_field] colors the entry field green when the word in it can be
     added to the successful words list. Colors is black otherwise. *)
  let color_word_field () =
    let text = W.get_text word_field in
    let color =
      if is_word_incomplete text then Draw.(opaque black)
      else Draw.(opaque green)
    in
    let lbl = W.get_label word_field in
    Label.set_fg_color lbl color
  in

  (* [add_letter l] appends [l] to [word_field]'s text and sets this string as
      its new text*)
  let add_letter l =
    let new_text = W.get_text word_field ^ l in
  ...
  (* [make_mini_board_layout] creates a board layout with labels corresponding
       to tiles on the board. Useful for display on results page. Returns the layout
       and also an array of labels for easy access for relabelling later *)
  let make_mini_board_layout () =
    ...
    let rec make_mini_rowi acc i j =
      if j < 0 then acc
      else
    ...
    let mini_rows = [] in
    let mini_rows = (make_mini_rowi [] 0 3 |> L.flat) :: mini_rows in
    let mini_rows = (make_mini_rowi [] 1 3 |> L.flat) :: mini_rows in
    let mini_rows = (make_mini_rowi [] 2 3 |> L.flat) :: mini_rows in
    let mini_rows = (make_mini_rowi [] 3 3 |> L.flat) :: mini_rows in

    (L.tower (List.rev mini_rows), labels)
  in

  let mini_board_layout, mini_board_labels = make_mini_board_layout () in
  ...
  let reset_text_field m =
    (let field_word = W.get_text word_field in
     if is_word_incomplete field_word then (
  ...
  let assign_label_letter i j brd =
    ...
    let _, board_label = board_matrix.(i).(j) in
    W.set_text board_label letter;
    let mini_label = mini_board_labels.(i).(j) in
    W.set_text mini_label letter
  in
  ...
  (*Page 3 *********************************************************************)
  let mini_board_title =
    W.rich_text ~size:20 ~w:width ~h:30
      Text_display.(page [ bold (para "Board") ])
  in
  ...
  let mini_board_with_title =
    L.tower [ L.resident ~h:80 mini_board_title; mini_board_layout ]
  in
  ...
  let display_lists =
    L.flat ~hmargin:200
      [ mini_board_with_title; top_words_list; top_user_word_list ]
  in
  ...
  let page3 =
    ...
    L.tower
      [
        ...
        L.resident ~w:1000 ~h:100 quit_results_button;
      ]
  in
  ...
  (* [set_remaining_time seconds] updated the countdown timer with the number of
     seconds left and also changes the color to red when the remaining seconds
     number is less or equal to a threshold *)
  let set_remaining_time seconds =
    let lbl = W.get_label timer_label in
    Label.set_fg_color lbl
      Draw.(if seconds <= seconds_to_warn then opaque red else opaque black);
    W.set_text timer_label
      (if seconds > 0 then "Time Left: " ^ string_of_int seconds
       else "Time is up");
  ...
  let start_button_normal_action input label _ =
    start_game ();
    switch_page Game;
    update_timer_normal normal_play_time
  in

  let start_button_survival_action input label _ =
    start_game ();
    switch_page Game;
    update_timer_survival survival_play_time
  in
  ...
  let quit_button_action _ _ _ = Sync.push (fun () -> raise Bogue.Exit) in

  let conns =
    W.connect quit_intro_button quit_intro_button quit_button_action
      Sdl.Event.[ mouse_button_down ]
    :: conns
  in

  let conns =
    W.connect quit_results_button quit_results_button quit_button_action
      Sdl.Event.[ mouse_button_down ]
    :: conns
  in
  ...
  (* print_endline (Print.layout_down tabs); *)
  let shortcuts =
    Bogue.shortcuts_of_list
      (if exit_on_escape then [ Bogue.exit_on_escape ] else [])
  in
  let board = Bogue.of_layout ~shortcuts ~connections:conns tabs in
  Bogue.run board
